...
async function checkAuthEmailOnly() {
  console.log('=== MZS Collection - auth_email Field Only ===\n');
  ...
  for (const wallet of walletAddresses) {
    try {
      ...
      if (!userSnapshot.empty) {
        ...
        // Check ONLY auth_email field
        const authEmail = userData.auth_email || 'NONE';
        
        console.log(`${wallet.name} - ${wallet.address}:`);
        console.log(`  auth_email: ${authEmail}`);
      } else {
        console.log(`${wallet.name} - ${wallet.address}:`);
        console.log('  auth_email: NOT FOUND IN DATABASE');
      }
    } catch (error) {
      console.log(`${wallet.name} - ${wallet.address}:`);
...
checkAuthEmailOnly().catch(error => {
